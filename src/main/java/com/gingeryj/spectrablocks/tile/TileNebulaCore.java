...
public class TileNebulaCore extends TileScalableEffect implements ITickable {

    private static final double RENDER_RADIUS = 4.85D;
    ...
    public void update() {
        ...
        double y = -0.75D + world.rand.nextDouble() * 1.5D;
        double horizontal = Math.sqrt(Math.max(0.0D, 1.0D - y * y));
        double radius = 2.2D + world.rand.nextDouble() * 2.2D;
